...
int add_replace_item(const char *orig_path, const char *new_path) {
    ...
    sprintf(src_env_name, "V_REPLACE_ITEM_SRC_%d", replace_item_count);//eg: src_env_name = "V_REPLACE_ITEM_SRC_2"
    sprintf(dst_env_name, "V_REPLACE_ITEM_DST_%d", replace_item_count);//eg: dst_env_name = "V_REPLACE_ITEM_DST_2"
    //setenv：设置环境变量
    setenv(src_env_name, orig_path, 1);
    setenv(dst_env_name, new_path, 1);

    //分配一个ReplaceItem数组 注意长度为：replace_item_count+1
    replace_items = (ReplaceItem *) realloc(replace_items,
                                            replace_item_count * sizeof(ReplaceItem) +
                                            sizeof(ReplaceItem));
    //获取ReplaceItem数组的最后一个元素
    ReplaceItem &item = replace_items[replace_item_count];
    item.orig_path = strdup(orig_path); //strdup复制字符串
    item.orig_size = strlen(orig_path); //strlen字符串长度
    item.new_path = strdup(new_path);
    item.new_size = strlen(new_path);
    item.is_folder = (orig_path[strlen(orig_path) - 1] == '/');
    return ++replace_item_count; //返回数组的下一个位置
}
...
const char *relocate_path(const char *_path, int *result) {
    ...
    for (int i = 0; i < replace_item_count; ++i) {
        ...
        ALOGE("替换路径：%s -> %s", item.orig_path, item.new_path);
        if (match_path(item.is_folder, item.orig_size, item.orig_path, path)) {
            ...
            *result = MATCH;
//            ALOGE("替换路径：%s -> %s", item.orig_path, redirect_path.c_str());
            free(path);
